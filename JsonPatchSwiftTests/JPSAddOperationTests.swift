...
class JPSAddOperationTests: XCTestCase {
    ...
    func testIfPathToNonExistingMemberCreatesNewMember1() {
        ...
        let expectedJson = JSON(data: "{ \"foo\" : \"bar\", \"baz\" : \"qux\" }".dataUsingEncoding(String.Encoding.utf8)!)
        XCTAssertEqual(resultingJson, expectedJson)
    }
    
    // http://tools.ietf.org/html/rfc6902#appendix-A.2
    func testIfPathToArrayCreatesNewArrayElement() {
        let json = JSON(data: " { \"foo\" : [ \"bar\", \"baz\"] } ".dataUsingEncoding(String.Encoding.utf8)!)
        let jsonPatch = try! JPSJsonPatch("{ \"op\": \"add\", \"path\": \"/foo/1\", \"value\": \"qux\" }")
        let resultingJson = try! JPSJsonPatcher.applyPatch(jsonPatch, toJson: json)
        let expectedJson = JSON(data: "{ \"foo\": [ \"bar\", \"qux\", \"baz\" ] }".dataUsingEncoding(String.Encoding.utf8)!)
        XCTAssertEqual(resultingJson, expectedJson)
    }
    
    func testIfPathToArrayInsertsValueAtPositionAndShiftsRemainingMembersRight() {
        let json = JSON(data: " [ \"foo\", 42, \"bar\" ] ".dataUsingEncoding(String.Encoding.utf8)!)
        let jsonPatch = try! JPSJsonPatch("{ \"op\": \"add\", \"path\": \"/2\", \"value\": \"42\" }")
        let resultingJson = try! JPSJsonPatcher.applyPatch(jsonPatch, toJson: json)
        let expectedJson = JSON(data: " [ \"foo\", 42, \"42\", \"bar\" ] ".dataUsingEncoding(String.Encoding.utf8)!)
        XCTAssertEqual(resultingJson, expectedJson)
    }
    
    func testIfPathToNonExistingMemberCreatesNewMember2() {
        let json = JSON(data: " { \"foo\" : \" { \"foo2\" : \"bar\" } \" } ".dataUsingEncoding(String.Encoding.utf8)!)
        let jsonPatch = try! JPSJsonPatch("{ \"op\": \"add\", \"path\": \"/foo/bar\", \"value\": \"foo\" }")
        let resultingJson = try! JPSJsonPatcher.applyPatch(jsonPatch, toJson: json)
        let expectedJson = JSON(data: " { \"foo\" : \" { \"foo2\" : \"bar\", \"bar\" : \"foo\" } \" } ".dataUsingEncoding(String.Encoding.utf8)!)
        XCTAssertEqual(resultingJson, expectedJson)
    }
    
    func testIfPathToNonExistingMemberCreatesNewMember3() {
        let json = JSON(data: " { \"foo\" : \" [ { \"foo\" : \"bar\" }, { \"blaa\" : \" { \" blubb \" : \"bloobb\" } \" } ] \" } ".dataUsingEncoding(String.Encoding.utf8)!)
        let jsonPatch = try! JPSJsonPatch("{ \"op\": \"add\", \"path\": \"/foo/1/blaa/blubby\", \"value\": \"foo\" }")
        let resultingJson = try! JPSJsonPatcher.applyPatch(jsonPatch, toJson: json)
        let expectedJson = JSON(data: " { \"foo\" : \" [ { \"foo\" : \"bar\" }, { \"blaa\" : \" { \" blubb \" : \"bloobb\", \"blubby\" : \"foo\" } \" } ] \" } ".dataUsingEncoding(String.Encoding.utf8)!)
        XCTAssertEqual(resultingJson, expectedJson)
    }
    ...
    func testIfPathToExistingMemberReplacesIt1() {
        ...
        let resultingJson = try! JPSJsonPatcher.applyPatch(jsonPatch, toJson: json)
        let expectedJson = JSON(data: "{ \"foo\" : \"foobar\" }".dataUsingEncoding(String.Encoding.utf8)!)
        XCTAssertEqual(resultingJson, expectedJson)
    }
    ...
    func testIfPathToExistingMemberReplacesIt2() {
        ...
        let resultingJson = try! JPSJsonPatcher.applyPatch(jsonPatch, toJson: json)
        let expectedJson = JSON(data: " { \"foo\" : \" [ { \"foo\" : \"bar\" }, { \"blaa\" : \" { \" blubb \" : \"foo\" } \" } ] \" } ".dataUsingEncoding(String.Encoding.utf8)!)
        XCTAssertEqual(resultingJson, expectedJson)
    }
    ...
    func testAddToArrayWithIndexOutOfBoundsProducesError() {
        do {
            let json = JSON(data: " { \"a\": [ 23, 42 ] } ".dataUsingEncoding(String.Encoding.utf8)!)
    ...
    func testAddToArrayWithIndexEqualsCount() {
        ...
        let resultingJson = try! JPSJsonPatcher.applyPatch(jsonPatch, toJson: json)
        let expectedJson = JSON(data: " { \"a\": [ 23, 42, \"bar\" ] } ".dataUsingEncoding(String.Encoding.utf8)!)
        XCTAssertEqual(resultingJson, expectedJson)
    }
    
    func testIfMinusAtEndOfPathAppendsToArray() {
        let json = JSON(data: " { \"foo\" : [ bar1, bar2, bar3 ] } ".dataUsingEncoding(String.Encoding.utf8)!)
        let jsonPatch = try! JPSJsonPatch("{ \"op\": \"add\", \"path\": \"/foo/-\", \"value\": \"bar4\" }")
        let resultingJson = try! JPSJsonPatcher.applyPatch(jsonPatch, toJson: json)
        let expectedJson = JSON(data: " { \"foo\" : [ bar1, bar2, bar3, bar4 ] } ".dataUsingEncoding(String.Encoding.utf8)!)
        XCTAssertEqual(resultingJson, expectedJson)
    }
    
    func testIfPathElementIsValid() {
        let json = JSON(data: " { \"a\": { \"foo\": 1 } } ".dataUsingEncoding(String.Encoding.utf8)!)
        let jsonPatch = try! JPSJsonPatch("{ \"op\": \"add\", \"path\": \"/a/b\", \"value\": \"bar\" }")
        let resultingJson = try! JPSJsonPatcher.applyPatch(jsonPatch, toJson: json)
        let expectedJson = JSON(data: " { \"a\": { \"foo\": 1, \"b\" : \"bar\" } } ".dataUsingEncoding(String.Encoding.utf8)!)
        XCTAssertEqual(resultingJson, expectedJson)
    }
